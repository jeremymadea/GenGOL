...
class CARules 
{
  ...
  CARules spawn() 
  {
    ...
    int rotpt_a = int(random(a.length()));
    int rotpt_b = int(random(b.length()));
    println("Rotation Point A: " + rotpt_a); 
    println("Rotation Point B: " + rotpt_b); 
    print_rule_set();
    rule_strings[0] = a.substring(rotpt_a, a.length()) + a.substring(0, rotpt_a);
    rule_strings[1] = b.substring(rotpt_b, b.length()) + b.substring(0, rotpt_b);
